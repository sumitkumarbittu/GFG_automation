...
test('manifest is MV3 with only required permissions and constrained hosts',()=>{assert.equal(manifest.manifest_version,3);assert.deepEqual(manifest.permissions.sort(),['alarms','sidePanel','storage','tabs'].sort());assert.deepEqual(manifest.host_permissions.sort(),['https://practice.geeksforgeeks.org/*','https://www.geeksforgeeks.org/*'].sort());assert.ok(Number(manifest.minimum_chrome_version)>=114)});
